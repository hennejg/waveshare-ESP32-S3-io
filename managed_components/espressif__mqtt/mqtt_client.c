...
esp_err_t esp_mqtt_client_start(esp_mqtt_client_handle_t client)
{
    ...
    if (xTaskCreatePinnedToCoreWithCaps(esp_mqtt_task, "mqtt_task", client->config->task_stack, client, client->config->task_prio, &client->task_handle, MQTT_TASK_CORE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT) != pdTRUE) {
        ESP_LOGE(TAG, "Error create mqtt task");
        err = ESP_FAIL;
    }
#else
    ESP_LOGD(TAG, "Core selection disabled");
    /* Use PSRAM for the task stack — after BLE+Matter init only ~1.5 KB of INTERNAL
     * heap remains (pvPortMalloc budget). xTaskCreateWithCaps puts the stack in PSRAM
     * while keeping the TCB in INTERNAL (pvPortMalloc inside xTaskCreateWithCaps). */
    if (xTaskCreateWithCaps(esp_mqtt_task, "mqtt_task", client->config->task_stack, client, client->config->task_prio, &client->task_handle, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT) != pdTRUE) {
        ESP_LOGE(TAG, "Error create mqtt task");
        err = ESP_FAIL;
    }
#endif
