...
**Transmit (echo):** Sent after every successful command. Format: opcode `0x00` (WRITE) + current full state bitmask, so any receiver can reconstruct the complete output state from a single frame.
...
- **DO writes via CAN** call the same `dout_set()` path as MQTT, so changes are automatically published on the corresponding MQTT `output/N` topic.
