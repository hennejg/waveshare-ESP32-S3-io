...
| Digital outputs (bulk) | `<prefix>/outputs/set` |
| LED | `<prefix>/led/set` |
...
| LED state | Meaning |
|-----------|---------|
| 50% yellow | Booted, no network yet |
| 50% green | Network (WiFi or Ethernet) |
| 50% purple | MQTT broker connected |
| 100% red flash (100 ms) | MQTT message received |
| 100% blue flash (100 ms) | MQTT message published |

### Input / Output Details
- **Invert flag** — each DI and DO has a configurable invert flag (web UI). Useful for active-low sensors or normally-closed contacts.
...
- **Coil writes via Modbus** — also publish the new state on the corresponding MQTT topic, keeping both interfaces in sync.
